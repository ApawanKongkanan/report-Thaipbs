...
return result.data;
    ...
    console.error("OCR ERROR FULL:", error);
...
document
  .getElementById("scanNoticeBtn")
  .addEventListener("click", async () => {

    const fileInput =
      document.querySelector('input[name="notice"]');

    const file = fileInput.files[0];

    if (!file) {
      alert("กรุณาเลือกไฟล์ใบแจ้งความ");
      return;
    }
    ...
    if (data.reporter?.phone) {
      document.querySelector('input[name="owner_phone"]').value =
        data.reporter.phone;
    }

    if (data.missing_person?.name) {
      document.querySelector('input[name="missing_name"]').value =
        data.missing_person.name;
    }

    if (data.missing_person?.age) {
      document.querySelector('input[name="age"]').value =
        data.missing_person.age;
    }

    alert("สแกนข้อมูลสำเร็จ");
});
